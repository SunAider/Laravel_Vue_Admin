...
class AuthService
{
    ...
    public function adminLogin($request)
    {
        ...
        if ($user->role_id !== RoleId::ADMIN && $user->role_id !== RoleId::subADMIN ) {
            abort(response()->json(['error_msg' => 'b管理者用ページにログインする権限のないアカウントです。'], HttpStatus::UNAUTHORIZED));
        }
        ...
        session(['currentUseerRole'=> $user->role_id]);
        Log::info("login", [$user, session('currentUseerRole')]);
        return $user;
    }
